...
──────────────────────────────────────────── */
function AppInsightsSidebar({ app }: { app: any }) {
  const hasInsights =
    app.aiInsights?.companyDescription ||
    app.aiInsights?.industry ||
    app.aiInsights?.businessModel;
  const hasCompetitors = app.competitors && app.competitors.length > 0;

  if (!hasInsights && !hasCompetitors) return null;

  return (
    <div className="flex flex-col gap-4">
      {hasInsights && (
        <div className="rounded-[16px] border border-[#EBEBEB] bg-[#FAFAFA] p-4 flex flex-col gap-3">
          <p className="font-secondary font-bold text-[12px] tracking-[0.1em] uppercase text-[#939393]">
            App Profile
          </p>

          {app.aiInsights.companyDescription && (
            <p className="font-secondary text-[13px] text-[#555] leading-[1.65]">
              {app.aiInsights.companyDescription}
            </p>
          )}
          ...
          <div className="flex flex-col gap-2 pt-1 border-t border-[#F0F0F0]">
            {app.aiInsights.industry && (
              <div className="flex items-start justify-between gap-2">
                <span className="font-secondary text-[11px] text-[#ACACAC] shrink-0">Industry</span>
                <span className="font-secondary text-[11px] font-semibold text-[#333] text-right">{app.aiInsights.industry}</span>
              </div>
            )}
            {app.aiInsights.businessModel && (
              <div className="flex items-start justify-between gap-2">
                <span className="font-secondary text-[11px] text-[#ACACAC] shrink-0">Model</span>
                <span className="font-secondary text-[11px] font-semibold text-[#333] text-right">{app.aiInsights.businessModel}</span>
              </div>
            )}
            {app.aiInsights.founded && app.aiInsights.founded !== "Unknown" && (
              <div className="flex items-start justify-between gap-2">
                <span className="font-secondary text-[11px] text-[#ACACAC] shrink-0">Founded</span>
                <span className="font-secondary text-[11px] font-semibold text-[#333] text-right">{app.aiInsights.founded}</span>
              </div>
            )}
            {app.aiInsights.headquarters && app.aiInsights.headquarters !== "Unknown" && (
              <div className="flex items-start justify-between gap-2">
                <span className="font-secondary text-[11px] text-[#ACACAC] shrink-0">HQ</span>
                <span className="font-secondary text-[11px] font-semibold text-[#333] text-right">{app.aiInsights.headquarters}</span>
              </div>
            )}
            {app.aiInsights.targetAudience && (
              <div className="flex items-start justify-between gap-2">
                <span className="font-secondary text-[11px] text-[#ACACAC] shrink-0">Audience</span>
                <span className="font-secondary text-[11px] font-semibold text-[#333] text-right">{app.aiInsights.targetAudience}</span>
              </div>
            )}
          </div>

          {app.aiInsights.keyFeatures && app.aiInsights.keyFeatures.length > 0 && (
            <div className="flex flex-col gap-1.5 pt-1 border-t border-[#F0F0F0]">
              <span className="font-secondary text-[11px] text-[#ACACAC]">Key Features</span>
              <div className="flex flex-col gap-1">
                {app.aiInsights.keyFeatures.map((f: string, i: number) => (
                  <div key={i} className="flex items-start gap-1.5">
                    <span className="text-purple-400 text-[10px] mt-0.5 shrink-0">•</span>
                    <span className="font-secondary text-[12px] text-[#444] leading-[1.5]">{f}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="flex items-center gap-1 pt-1 border-t border-[#F0F0F0]">
            <span className="text-[10px] text-[#CFCFCF] font-secondary">Generated by AI</span>
          </div>
        </div>
      )}
      ...
      {hasCompetitors && (
        <div className="rounded-[16px] border border-[#EBEBEB] bg-[#FAFAFA] p-4 flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-1">
            {app.competitors.map((competitor: any) => (
              <Link
                key={competitor.id}
                to={`/app-v2/${competitor.id}`}
                className="flex items-center gap-3 px-2 py-2 rounded-[10px] hover:bg-white transition-colors group"
              >
                <ImageWithFallback
                  ...
                  className="w-8 h-8 rounded-[8px] object-contain border border-[#F0F0F0]"
                />
                <span className="font-secondary text-[13px] font-semibold text-[#0F0F0F] truncate group-hover:text-purple-600 transition-colors">
                  {competitor.name}
                </span>
                <ChevronRight className="w-3.5 h-3.5 text-[#D0D0D0] ml-auto shrink-0 group-hover:text-purple-400 transition-colors" />
              </Link>
            ))}
          </div>
        </div>
...
const AppDetailV2: React.FC = () => {
  ...
  return (
    <>
      ...
      {/* ─── App Profile (AI Insights) ─── */}
      {import.meta.env.VITE_ENABLE_AI_INSIGHTS === "true" && app.aiInsights?.companyDescription && (
        <div className="w-full bg-[#111] border-b border-white/[0.06]">
          <div className="w-full flex justify-center px-4 md:px-0">
            <div className="w-full max-w-[1200px] py-6 flex flex-col gap-4">
              <span className="font-secondary text-[11px] font-semibold tracking-[0.14em] uppercase text-white/25">
                App Profile
              </span>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-6 gap-y-3">
                {app.aiInsights.industry && (
                  <div className="flex flex-col gap-0.5">
                    <span className="font-secondary text-[10px] text-white/30 uppercase tracking-[0.1em]">Industry</span>
                    <span className="font-secondary text-[13px] font-semibold text-white/75">{app.aiInsights.industry}</span>
                  </div>
                )}
                {app.aiInsights.businessModel && (
                  <div className="flex flex-col gap-0.5">
                    <span className="font-secondary text-[10px] text-white/30 uppercase tracking-[0.1em]">Model</span>
                    <span className="font-secondary text-[13px] font-semibold text-white/75">{app.aiInsights.businessModel}</span>
                  </div>
                )}
                {app.aiInsights.founded && app.aiInsights.founded !== "Unknown" && (
                  <div className="flex flex-col gap-0.5">
                    <span className="font-secondary text-[10px] text-white/30 uppercase tracking-[0.1em]">Founded</span>
                    <span className="font-secondary text-[13px] font-semibold text-white/75">{app.aiInsights.founded}</span>
                  </div>
                )}
                {app.aiInsights.headquarters && app.aiInsights.headquarters !== "Unknown" && (
                  <div className="flex flex-col gap-0.5">
                    <span className="font-secondary text-[10px] text-white/30 uppercase tracking-[0.1em]">HQ</span>
                    <span className="font-secondary text-[13px] font-semibold text-white/75">{app.aiInsights.headquarters}</span>
                  </div>
                )}
              </div>
              ...
              {app.aiInsights.targetAudience && (
                <div className="flex flex-col gap-0.5">
                  <span className="font-secondary text-[10px] text-white/30 uppercase tracking-[0.1em]">Target Audience</span>
                  <span className="font-secondary text-[13px] text-white/55 leading-[1.6]">{app.aiInsights.targetAudience}</span>
                </div>
              )}
              ...
              {app.aiInsights.keyFeatures && app.aiInsights.keyFeatures.length > 0 && (
                <div className="flex flex-col gap-1.5">
                  <span className="font-secondary text-[10px] text-white/30 uppercase tracking-[0.1em]">Key Features</span>
                  <div className="flex flex-wrap gap-2">
                    {app.aiInsights.keyFeatures.map((f: string, i: number) => (
                      <span key={i} className="font-secondary text-[12px] text-white/60 bg-white/[0.06] border border-white/10 px-3 py-1 rounded-full">
                        {f}
                      </span>
                    ))}
                  </div>
              ...
              <span className="font-secondary text-[10px] text-white/20">Generated by AI</span>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="bg-white min-h-[60vh]">
        {/* ─── Competitors ─── */}
        {import.meta.env.VITE_ENABLE_AI_INSIGHTS === "true" && app.competitors && app.competitors.length > 0 && (
          <div className="w-full border-b border-[#F0F0F0]">
            <div className="w-full flex justify-center px-4 md:px-0">
              <div className="w-full max-w-[1200px] py-3 flex items-center gap-3 overflow-x-auto [&::-webkit-scrollbar]:hidden [scrollbar-width:none]">
                <span className="font-secondary text-[11px] font-semibold text-[#BDBDBD] uppercase tracking-[0.12em] shrink-0">
                  Competitors
                </span>
                <div className="w-px h-4 bg-[#EBEBEB] shrink-0" />
                {app.competitors.map((c: any) => (
                  <Link
                    key={c.id}
                    to={`/app-v2/${c.id}`}
                    className="flex items-center gap-2 shrink-0 h-8 px-3 rounded-full border border-[#EBEBEB] bg-[#F8F8F8] hover:border-purple-300 hover:bg-white transition-all group"
                  >
                    <ImageWithFallback
                      src={c.image}
                      fallbackSrc="https://placehold.co/400"
                    ...
                    />
                    <span className="font-secondary text-[12px] font-semibold text-[#444] group-hover:text-purple-600 transition-colors whitespace-nowrap">
                      {c.name}
                    </span>
                  </Link>
                ))}
