...
const UX_PATTERNS = {
  'onboarding': ['welcome', 'tutorial', 'getting started', 'first time', 'intro', 'setup', 'walkthrough'],
  'empty state': ['no content', 'no data', 'empty', 'placeholder', 'no results', 'nothing here'],
  'error state': ['error', 'failed', 'something went wrong', 'try again', 'oops', '404'],
  'loading state': ['loading', 'spinner', 'skeleton', 'progress', 'buffering'],
  'checkout': ['payment', 'billing', 'cart', 'purchase', 'order', 'buy now', 'checkout'],
...
export const performEnhancedSearch = (apps: App[], searchTerm: string) => {
  ...
  return apps.filter(app => {
    ...
    const screenMatch = app.screens.some(screen => 
    ...
    // UX Pattern matching
    const uxPatternMatch = Object.entries(UX_PATTERNS).some(([pattern, keywords]) => {
      const patternMatch = term.includes(pattern) || keywords.some(keyword => term.includes(keyword));
      if (patternMatch) {
    ...
    const industryMatch = Object.entries(INDUSTRY_PATTERNS).some(([industry, keywords]) => {
      const industryTermMatch = term.includes(industry) || keywords.some(keyword => term.includes(keyword));
    ...
    // Platform matching
    const platformMatch = Object.entries(PLATFORM_PATTERNS).some(([platform, keywords]) => {
      const platformTermMatch = term.includes(platform) || keywords.some(keyword => term.includes(keyword));
      if (platformTermMatch) {
        return app.platform.toLowerCase().includes(platform) ||
          keywords.some(keyword => 
            app.description.toLowerCase().includes(keyword) ||
            app.tags.some(tag => tag.toLowerCase().includes(keyword))
          );
    ...
    return basicMatch || screenMatch || uxPatternMatch || industryMatch || platformMatch || semanticMatch;
  });
};
...
const detectSemanticQueries = (term: string, app: App): boolean => {
  ...
  if (howDoesMatch) {
    ...
    const featureMatch = app.screens.some(screen => 
